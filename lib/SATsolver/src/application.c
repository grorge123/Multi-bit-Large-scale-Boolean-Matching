...
static void print_complete_usage(void) {
    ...
    printf("\n");
    printf(
        "Further '<option>' can be one of the "
...
#endif
    printf(
        "  --relaxed            relaxed parsing"
        " (ignore DIMACS header)\n");
    printf(
        "  --strict             stricter parsing"
...
static const char* single_first_option_table[] = {"-h",         "--help",
                                                  "--banner",   "--compiler",
#ifndef NOPTIONS
                                                  "--embedded",
#endif
        ...
        if (single_first_option(arg))
            ...
            if (kissat_parse_option_value(valstr, &val) && val > 0) {
                if (application->time > 0)
                    ERROR("multiple '--time=%d' and '%s'", application->time,
                          arg);
...
#else
            ERROR(
                "two file arguments '%s' and '%s' without proof support "
                "(try '-h')",
...
static bool parse_input(application* application) {
    ...
    if (file.compressed) {
        ...
        kissat_message(solver, "inflated input file of size %s by %.2f",
                       FORMAT_BYTES(bytes), kissat_average(file.bytes, bytes));
    }
...
static void print_option(kissat* solver, int value, const opt* o) {
    ...
    kissat_message(solver, "--%-30s (%s default '%s')", buffer,
                   (value == o->value ? "same as" : "different from"), def_str);
}
#endif
...
solverResult kissat_application(kissat* solver, int argc, char** argv) {
    solverResult result;
...
//            printf("s SATISFIABLE\n");
            result.satisfiable = 1;
//            fflush(stdout);
            result.inputSize = application.max_var;
            result.input = kissat_print_witness(solver, application.max_var,
                                 application.partial);
        }
    }
    return result;
